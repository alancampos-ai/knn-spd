...
We present a computational study of supervised voxel-wise classification of diffusion tensor MRI (DT-MRI). Each voxel is represented by a 3×3 symmetric positive definite (SPD) matrix, and classification is performed directly on SPD(3) using *k*-nearest neighbors. The study compares Frobenius (FR), Log-Euclidean (LE), affine-invariant Riemannian (AIRM), and deterministic border–interior hybrid classifiers under a unified experimental design.
